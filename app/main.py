...
def create_exercise_log(log: ExerciseLog, user_email: str = CurrentUser):
    ...
    from app.db import AnalyticsDB
    AnalyticsDB.invalidate_user_cache(user_email, ["performance_predictions", "correlations"])
...
try:
    ...
    class MinimalPredictionService:
        ...
        def generate_intervention_suggestions(self, user_email):
            return []
        def log_prediction_accuracy(self, user_email, prediction_type, predicted_value, actual_value, prediction_date, actual_date=None):
...
def get_analytics_insights(days: int = 30, user_email: str = CurrentUser):
    ...
    try:
        ...
        weight_trends = analytics_service.calculate_weight_trends(user_email, days)
        macro_trends = analytics_service.analyze_macro_patterns(user_email, days)
        correlations = analytics_service.correlate_nutrition_performance(user_email, min(days * 2, 90))
...
def submit_background_analysis(analysis_type: str, days: int = 90, user_email: str = CurrentUser):
    """Submit expensive analysis for background processing"""
    try:
        valid_types = ["comprehensive_correlation", "long_term_trends", "performance_modeling"]
        ...
        task_id = analytics_service.submit_expensive_analysis(user_email, analysis_type, days=days)
...
def clear_user_cache(analysis_types: Optional[List[str]] = None, user_email: str = CurrentUser):
    """Clear analytics cache for current user"""
    try:
        from app.db import AnalyticsDB
        cleared_count = AnalyticsDB.invalidate_user_cache(user_email, analysis_types or [])
...
def get_prediction_accuracy_metrics(prediction_type: Optional[str] = None, user_email: str = CurrentUser):
    ...
    try:
        ...
        accuracy_stats = prediction_service.get_prediction_accuracy_stats(user_email, prediction_type_str)
        
        # Get accuracy trends over time
        accuracy_trends = prediction_service.calculate_accuracy_trends(user_email, prediction_type_str)
        
        # Get performance by prediction type
        type_performance = prediction_service.get_prediction_type_performance(user_email)
        ...
        return {
            ...
            "analysis_summary": {
                ...
                "most_accurate_type": max(type_performance.keys(), 
                                        key=lambda k: type_performance[k]["average_accuracy"]) if type_performance else None,
                "least_accurate_type": min(type_performance.keys(), 
                                         key=lambda k: type_performance[k]["average_accuracy"]) if type_performance else None
            },
            "generated_at": datetime.now().isoformat()
        }
...
def log_prediction_accuracy(
    ...
    try:
        ...
        success = prediction_service.log_prediction_accuracy(
            ...
            prediction_date, actual_date_str
        )
