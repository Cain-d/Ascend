...
class PerformanceTestSuite:
    ...
    def setup_large_dataset(self, days: int = 365, daily_entries: int = 5):
        ...
        with get_db_connection() as conn:
            ...
            conn.commit()
            cursor.execute("SELECT id FROM users WHERE email = ?", (self.test_user_email,))
            ...
            for i in range(0, days, 2):  # Every other day
                ...
                for exercise_num in range(random.randint(3, 6)):
                    ...
                    for set_num in range(1, sets + 1):
                        reps = base_reps + random.randint(-2, 2)  # Slight variation per set
                        weight = base_weight
    ...
    def test_cache_performance(self) -> Dict[str, Any]:
        ...
        weight_trends_1 = self.analytics_service.calculate_weight_trends(self.test_user_email, 90)
        first_call_time = time.time() - start_time
        results["cache_misses"].append(first_call_time)
        
        # Test 2: Second call (cache hit)
        start_time = time.time()
        weight_trends_2 = self.analytics_service.calculate_weight_trends(self.test_user_email, 90)
        second_call_time = time.time() - start_time
        results["cache_hits"].append(second_call_time)
        
        # Test 3: Macro patterns (cache miss)
        start_time = time.time()
        macro_trends_1 = self.analytics_service.analyze_macro_patterns(self.test_user_email, 90)
        macro_first_time = time.time() - start_time
        results["cache_misses"].append(macro_first_time)
        
        # Test 4: Macro patterns (cache hit)
        start_time = time.time()
        macro_trends_2 = self.analytics_service.analyze_macro_patterns(self.test_user_email, 90)
        ...
        speedup_factor = avg_miss_time / avg_hit_time if avg_hit_time > 0 else float('inf')
        
        results["cache_effectiveness"] = {
            "avg_cache_miss_time": round(avg_miss_time, 4),
            "avg_cache_hit_time": round(avg_hit_time, 4),
            "speedup_factor": round(speedup_factor, 2),
            "cache_efficiency_percent": round((1 - avg_hit_time/avg_miss_time) * 100, 2)
        }
        
        print(f"   Cache miss avg: {avg_miss_time:.4f}s")
        print(f"   Cache hit avg: {avg_hit_time:.4f}s")
        print(f"   Speedup factor: {speedup_factor:.2f}x")
        print(f"   Cache efficiency: {results['cache_effectiveness']['cache_efficiency_percent']:.1f}%")
    ...
    def test_large_dataset_performance(self) -> Dict[str, Any]:
        ...
        for days in dataset_sizes:
            ...
            # Test weight trends
            start_time = time.time()
            weight_trends = self.analytics_service.calculate_weight_trends(self.test_user_email, days)
            weight_time = time.time() - start_time
            
            # Test macro patterns
            start_time = time.time()
            macro_trends = self.analytics_service.analyze_macro_patterns(self.test_user_email, days)
            macro_time = time.time() - start_time
            
            # Test correlations
            start_time = time.time()
            correlations = self.analytics_service.correlate_nutrition_performance(self.test_user_email, days)
        ...
        if len(dataset_sizes) >= 2:
            small_total = results["execution_times"][f"{dataset_sizes[0]}_days"]["total"]
            large_total = results["execution_times"][f"{dataset_sizes[-1]}_days"]["total"]
            ...
            results["scalability_metrics"] = {
                ...
                "scalability_efficiency": round(data_size_ratio / time_ratio, 2) if time_ratio > 0 else 0,
                "linear_scalability_score": round(min(1.0, data_size_ratio / time_ratio), 2) if time_ratio > 0 else 0
            }
    ...
    def test_cache_invalidation(self) -> Dict[str, Any]:
        ...
        invalidated_count = self.analytics_db.invalidate_user_cache(self.test_user_email)
        results["invalidation_tests"].append({
            "type": "full_user",
            "target": "all",
            "invalidated_count": invalidated_count
        ...
        print(f"   ✅ Cache invalidation tests completed")
    ...
    def run_all_tests(self, dataset_days: int = 365) -> Dict[str, Any]:
        ...
        try:
            ...
            all_results["results"]["large_dataset_performance"] = self.test_large_dataset_performance()
            all_results["results"]["background_processing"] = self.test_background_processing()
            all_results["results"]["cache_invalidation"] = self.test_cache_invalidation()
    ...
    def generate_test_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "cache_performance" in results:
            ...
            if "cache_effectiveness" in cache_result:
                summary["performance_metrics"]["cache_speedup"] = cache_result["cache_effectiveness"]["speedup_factor"]
                summary["performance_metrics"]["cache_efficiency"] = cache_result["cache_effectiveness"]["cache_efficiency_percent"]
        ...
        if "large_dataset_performance" in results:
            ...
            if "scalability_metrics" in dataset_result:
                summary["performance_metrics"]["scalability_score"] = dataset_result["scalability_metrics"]["linear_scalability_score"]
        
        # Generate recommendations
        if summary["performance_metrics"].get("cache_speedup", 0) < 2:
            summary["recommendations"].append("Consider increasing cache duration for better performance")
        
        if summary["performance_metrics"].get("scalability_score", 1) < 0.5:
            summary["recommendations"].append("Performance degrades significantly with large datasets - consider optimization")
        
        if summary["failed_tests"] > 0:
            summary["recommendations"].append("Some tests failed - review error logs and fix issues")
        ...
        try:
            with get_db_connection() as conn:
                ...
                if user_row:
                    ...
                    cursor.execute("DELETE FROM meal_items WHERE meal_id IN (SELECT id FROM meals WHERE user_id = ?)", (user_id,))
                    cursor.execute("DELETE FROM meals WHERE user_id = ?", (user_id,))
                    cursor.execute("DELETE FROM bodyweight_log WHERE user_id = ?", (user_id,))
                    cursor.execute("DELETE FROM exercise_logs WHERE workout_id IN (SELECT id FROM workouts WHERE user_id = ?)", (user_id,))
                    cursor.execute("DELETE FROM workouts WHERE user_id = ?", (user_id,))
                    cursor.execute("DELETE FROM performance_metrics WHERE user_id = ?", (user_id,))
                ...
                cursor.execute("DELETE FROM analytics_cache WHERE user_email = ?", (self.test_user_email,))
                cursor.execute("DELETE FROM prediction_accuracy WHERE user_email = ?", (self.test_user_email,))
                conn.commit()
