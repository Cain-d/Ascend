...
def test_real_database_integration():
    ...
    for pred_type, pred_val, actual_val, pred_date in test_predictions:
        ...
        print(
            f"   Logged {pred_type}: predicted={pred_val}, actual={actual_val}, success={result}"
        )
    ...
    workout_stats = service.get_prediction_accuracy_stats(
        test_user, "workout_performance"
    )
    ...
    for record in recent[:3]:  # Show first 3
        print(
            f"   - {record['prediction_type']}: {record['predicted_value']} → {record['actual_value']} (accuracy: {record['accuracy_score']:.3f})"
        )
